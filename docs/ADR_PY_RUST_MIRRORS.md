...
| `coord/smoke.py:251` | `src/app/pipeline.rs::test_stage_status_for` | Not a re-derived rule: the shared *assumption* that exactly one smoke assignment represents one Test-stage attempt. Python holds it in `coord.claim`'s branch-scoped smoke dedupe, `coord.stage_projection._leg_count_for_stage` and `coord.notify._record_smoke_verdict`; the Rust badge derivation holds it too. Listed because it is a hard-coded reference to a coord-tui source file, and because the #3177 Test-stage fan-out that would retire it cannot land on one side alone — until `pipeline.rs` is taught otherwise, N concurrent smoke legs render as N retries. |
